    ...
    def select(self):
        ...
        for n in range(self.min_n_components, self.max_n_components + 1):
            try:
                ...
                logl = model.score(self.X, self.lengths)  # l = Likelihod of the fitted model
                # No. of parameters = Initial state occupation probabilities + Transition probabilities + Emission probabilities
                # p = init_state_prob + trans_prob + em_prob
                # = n + [n * (n - 1)] + (n * features * 2); features = no. of data pts
                # = n**2 + (2 * n * features)
...
class SelectorDIC(ModelSelector):
    ...
    where X is the training data in dictionary form and X(i) is the current word 
    '''
        ...
        for n in range(self.min_n_components, self.max_n_components+1):
            ...
            try:
                ...
                for word in self.words:
                    ...
                    avg_anti_logL += model.score(other_X, other_lengths)
                    count_other_words += 1
                
                    avg_anti_logL /= count_other_words
                    dic_score = current_word_logL - avg_anti_logL
...
class SelectorCV(ModelSelector):
    ...
    def select(self):
        ...
        logL = []  # list of cross validation scores
        best_n = self.min_n_components
        ...
        for n in range(self.min_n_components, self.max_n_components+1):
            ...
            # Calculate average log-likelihood on cv test splits
            avg_logL = np.average(scores) if len(scores) > 0 else float("-inf")
